...
def init_routes(app):
    ...
    def ver_bitacora(project_id):
        ...
        registros = ProjectLog.query.filter_by(project_id=project.id).order_by(ProjectLog.timestamp.desc()).all()
        registros = ProjectLog.query.filter_by(project_id=project.id).order_by(ProjectLog.timestamp.asc()).all()

        return render_template('bitacora_ver.html', project=project, registros=registros)
...
@login_required
def cronograma():
    all_projects = Project.query.order_by(Project.start_date).all()

    if all_projects:
        min_date = min(p.start_date for p in all_projects)
        max_date = max(p.end_date for p in all_projects)
        total_days = (max_date - min_date).days or 1

        for p in all_projects:
            p.duration_percent = (p.end_date - p.start_date).days / total_days * 100
            p.days_offset = (p.start_date - min_date).days / total_days * 100
    else:
        for p in all_projects:
            p.duration_percent = 0
            p.days_offset = 0

    return render_template("cronograma.html", projects=all_projects)
