...
def init_routes(app):
    ...
    def cronograma():
        ...
        def create_task(project_id):
            ...
            # Solo usuarios participantes pueden crear tareas
            if not any(p.user_id == current_user.id for p in project.participants) and current_user.role != 'admin':
            ...
            if form.validate_on_submit():
                ...
                return redirect(url_for('project_tasks', project_id=project_
    ...
    def project_tasks(project_id):
        ...
        # Mostrar todas las tareas si el usuario tiene acceso al proyecto
        if any(p.user_id == current_user.id for p in project.participants) or current_user.role == 'admin':
            tasks = Task.query.filter_by(project_id=project.id).all()
            return render_template("project_tasks.html", project=project, tasks=tasks)
        else:
    ...
    def create_task(project_id):
        ...
        # 🔽 Aquí es donde debes pegar la línea
        form.assigned_to_id.choices = [
            (u.user.id, u.user.name) for u in project.participants if u.user.role == 'participant'
        ]
    ...
    def edit_task(task_id):
        ...
        if current_user.role not in ['leader', 'admin']:
            flash("Solo líderes o administradores pueden editar tareas.")
            return redirect(url_for('project_tasks', project_id=task.project_id))
