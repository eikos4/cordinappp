...
class Task(db.Model):
    ...
    description = db.Column(db.Text)
    estimated_hours = db.Column(db.Float)  # Tiempo estimado
    created_by_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    assigned_to_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # 🔧 Falta esta

    created_by = db.relationship('User', foreign_keys=[created_by_id], backref='tasks_created')
    assigned_to = db.relationship('User', foreign_keys=[assigned_to_id])  # 🔧 Arreglo aquí
