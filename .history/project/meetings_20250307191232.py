...
def list_meetings():
    page = request.args.get('page', 1, type=int)  # Obtiene la página actual desde la URL
    per_page = 5  # Número de reuniones por página

    meetings_pagination = Meeting.query.order_by(Meeting.date).paginate(page=page, per_page=per_page, error_out=False)

    return render_template("meetings/list_meetings.html", meetings_pagination=meetings_pagination)
