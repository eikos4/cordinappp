...
def list_tasks():
    ...
    # Filtros desde los parámetros del request
    ...
    in_progress_tasks = query.filter(Task.status == "En curso").count()
    completed_tasks = query.filter(Task.status == "Finalizada").count()

    tasks_by_responsible = defaultdict(list)
    for task in tasks_all:
        responsible = User.query.get(task.assigned_to).username if task.assigned_to else "Sin asignar"
    ...
    return render_template(
        ...
        tasks_all=tasks_all,
        tasks_pagination=tasks_pagination,  # ← Variable añadida aquí
