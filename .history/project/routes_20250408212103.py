...
@tasks_bp.route('/timeline_month')
def timeline_month():
    # Lógica para obtener tus tareas, p.ej. mes=4, year=2025
    year = 2025
    month = 4

    # tasks_all = ... (consulta a tu base de datos)
    # Ejemplo:
    tasks_all = Task.query.filter(/* tu lógica de fecha */).all()

    return render_template('timeline_month.html',
                           tasks_all=tasks_all,
                           year=year,
                           month=month)

# Asegúrate de registrar este blueprint en tu app principal
