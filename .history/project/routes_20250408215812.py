...
def show_calendar():
    # 1) Determinar el mes/año a mostrar
    #    Ejemplo: el mes actual
    now = datetime.now()
    year = now.year
    month = now.month
    # Opcional: si quieres que el usuario seleccione el mes
    # year = int(request.args.get("year", now.year))
    # month = int(request.args.get("month", now.month))

    # 2) Usar el módulo calendar para saber:
    #    - El primer día de la semana en que cae
    #    - El número total de días
    first_weekday, total_days = calendar.monthrange(year, month)
    # Esto te da, por ejemplo, (5, 31) si es un mes con 31 días que empieza en sábado (weekday=5)

    # 3) Obtener tus tareas
    #    Suponiendo que tienes un modelo Task con start_date en la DB, filtra al mes actual:
    from_date = datetime(year, month, 1)
    # Para el último día, debemos tener cuidado en meses con 28/29/30/31
    to_date = datetime(year, month, total_days, 23, 59, 59)
    ...
    # 4) Preparar info para el template
    return render_template(
        "calendar.html",
        tasks_all=tasks_all,
        year=year,
