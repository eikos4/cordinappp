...
def init_routes(app):
    ...
    def dashboard():
        ...
        if role == 'admin':
            ...
            tasks_by_project = {p.id: Task.query.filter_by(project_id=p.id).all() for p in projects}
            return render_template('dash_admin.html', users=users, projects=projects, tasks_by_project=tasks_by_project)

        elif role == 'leader':
            leader_projects = [pu.project for pu in current_user.projects]
            tasks_by_project = {
                p.id: Task.query.filter_by(project_id=p.id).all() for p in leader_projects
            }
    ...
    def admin_metrics():
        ...
        role_counts = {
            ...
            'leader': sum(1 for u in users if u.role == 'leader'),
            'participant': sum(1 for u in users if u.role == 'participant')
        }

        # Horas por usuario
        user_hours = {}
        for user in users:
            hours = sum(pu.hours_worked for pu in user.projects)
            user_hours[user.name] = hours

        # Horas por proyecto + estado + fechas + progreso
        project_hours = {}
        project_status = {}
        project_dates = {}
        project_progress = {}
        ...
        for project in projects:
            project_hours[project.name] = sum(pu.hours_worked for pu in project.participants)
            project_status[project.name] = project.status
            project_dates[project.name] = {
            ...
            project_progress[project.name] = project.progress or 0
    ...
    def user_profile():
        ...
        # Solo pasa todos los usuarios si es administrador
    ...
    def project_tasks(project_id):
        ...
        # Verificar que el usuario tenga relación con el proyecto
        if any(p.user_id == current_user.id for p in project.participants) or current_user.role == 'admin':
            tasks = Task.query.filter_by(project_id=project.id).all()
            return render_template('project_tasks.html', project=project, tasks=tasks)
        
        flash("No tienes permisos para ver las tareas de este proyecto", "warning")
        return redirect(url_for("dashboard"))
    ...
    def update_task_status(task_id, new_status):
        ...
        if current_user.id != task.assigned_to_id and current_user.role not in ['leader', 'admin']:
    ...
    def edit_task(task_id):
        ...
        if current_user.id != task.assigned_to_id and current_user.role not in ['leader', 'admin']:
            flash("Solo puedes editar tareas que te han sido asignadas.")
            return redirect(url_for('project_tasks', project_id=task.project_id))

        form = TaskForm(obj=task)
        form.assigned_to_id.choices = [(u.id, u.name) for u in User.query.all()]
        ...
        if form.validate_on_submit():
            ...
            task.real_hours = form.real_hours.data
            ...
            for pu in task.project.participants:
                if pu.user_id != current_user.id:
                    crear_notificacion(
                        pu.user_id,
                        f"{current_user.name} editó la tarea '{task.name}' del proyecto '{task.project.name}'"
                    )
